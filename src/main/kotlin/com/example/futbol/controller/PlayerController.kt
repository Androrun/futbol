...
class PlayerController {
    ...
    @GetMapping("/cup")
    fun listTotals (): List<Player>? {
        return playerService.findAvailablePlayer()
    }

    @GetMapping("/ages/{age}")
    fun listAges (@PathVariable("age") age: Int? ):ResponseEntity<*>{
        return ResponseEntity(playerService.findTotalPlayer(age), HttpStatus.OK)
    }
}